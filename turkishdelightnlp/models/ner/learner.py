...
class NERModel(BaseModel):
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("words"):
            if self.config.use_pretrained is None:
                ...
                # Multiple embeddings are used at the same time, we should concatenate
                # them
            ...
            if self.config.use_chars is not None:
                ...
                if self.config.use_chars == 'cnn':
                    ...
                    pool2 = tf.layers.average_pooling1d(
                        inputs=conv2, pool_size=2, strides=1)

                    # Dense Layer
                    output = tf.layers.dense(
                        inputs=pool2, units=32, activation=tf.nn.relu)
                    ...
                    word_embeddings.set_shape((None, None, 730))
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("bi-lstm"):
            cell_fw = tf.contrib.rnn.LSTMCell(
                self.config.hidden_size_lstm, activation=tf.nn.relu)
            ...
            output = tf.layers.dense(
                inputs=output,
                units=self.config.hidden_size_lstm * 2,
                activation=None)
        ...
        with tf.variable_scope("proj"):
            ...
            self.logits = tf.reshape(tf.matmul(output, W_1) + b_1,
                                     [-1, nsteps, self.config.ntags])
            self.logits2 = tf.reshape(tf.matmul(output, W_2) +
                                      b_2, [-1, nsteps, self.config.ntags])
    ...
    def run_evaluate(self, test, print_to_file=False):
        ...
        for words, labels in minibatches(test, self.config.batch_size):
            labels_pred, sequence_lengths, fd = self.predict_batch(
                words, return_feed=True)
    ...
    def predict_ner(self, words_raw):
