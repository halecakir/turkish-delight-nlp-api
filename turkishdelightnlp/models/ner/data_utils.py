...
class CoNLLDataset(object):
    ...
    def __init__(
        self, filename, processing_word=None, processing_tag=None, max_iter=None
    ):
        """
        Args:
...
def export_trimmed_word_vectors(
    vocab, vec_filename, trimmed_filename, dim, partial_match=False
):
    ...
    with open(vec_filename) as f:
        for line in f:
            ...
            if partial_match:
                _matching = [s for s in vocab if str(s).endswith(word)]
                if not _matching:
                    continue
    ...
    print(
        "Found {} words in the pre-trained embedding file. {} number of them in dataset vocabulary.".format(
            numb_of_words, numb_of_words_in_vocab
        )
    )
        ...
        if vocab_words is not None:
            if word in vocab_words:
                ...
                if allow_unk:
                    ...
                    raise Exception(
                        "Unknown key is not allowed. Check that "
...
def _pad_sequences(sequences, pad_tok, max_length):
    ...
    for seq in sequences:
        seq = list(seq)
        seq_ = (
            seq[:max_length] + [pad_tok] * max(max_length - len(seq), 0)
...
def pad_sequences(sequences, pad_tok, nlevels=1, max_len=-1):
    ...
    if nlevels == 1:
        ...
        max_length_word = (
            max([max(map(lambda x: len(x), seq)) for seq in sequences])
            if max_len == -1
            else max_len
        )
        sequence_padded, sequence_length = [], []
    ...
    for i, tok in enumerate(seq):
        ...
        # End of a chunk + start of a chunk!
            ...
            if chunk_type is None:
                ...
                tok_chunk_type != chunk_type
                or tok_chunk_class == "B"
                or tok_chunk_class == "S"
            ):
                chunk = (chunk_type, chunk_start, i)
                chunks.append(chunk)
                chunk_type, chunk_start = tok_chunk_type, i
